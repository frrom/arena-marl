...
class AGENT_26(BaseAgent):
    ...
    net_arch = [dict(pi=[64, 128, 32], vf=[64, 128, 32])]
    activation_fn = nn.ReLU
...
class AGENT_27(BaseAgent):
    ...
    features_extractor_kwargs = dict(features_dim=512)
    net_arch = [256, dict(pi=[128, 128, 64], vf=[128, 128, 64])]
    activation_fn=nn.ReLU
...
class AGENT_28(BaseAgent):
    type = PolicyType.CNN
    features_extractor_class = PSEUDO_RNN_EXTRACTOR
    features_extractor_kwargs = dict(features_dim=512)
    net_arch = [256, dict(pi=[128, 128, 64], vf=[128, 128, 64])]
    activation_fn=nn.ReLU
...
#     net_arch = [128, dict(pi=[64, 64], vf=[64, 64])]
#     policy_kwargs = dict(activation_fn=nn.ReLU, net_arch=[128, dict(pi=[64, 64], vf=[64, 64])],
#                          use_sde=False, log_std_init=-1, ortho_init=False, rnn_type="lstm",
#                          rnn_hidden_size=(128, 64), full_std=False)
...
#     features_extractor_class = EXTRACTOR_7
#     features_extractor_kwargs = dict(features_dim=300)
#     net_arch = [dict(pi=[32, 32], vf=[32, 32]), {'lstm': {'units': 64, 'use_layer_norm': True, 'layers': 2}}]
